...
class ChatProvider extends React.Component {
  ...
  receiveMessage() {
    socket.on('receive private message', (msg) => {
      ...
      const alreadyHasMessages = usersMessages.some(
        (x) =>
          x.conversationId === msg[this.emitterOrReceiverAttr(isOwnMessage)]
      )
  ...
  populatePrivateMessages({ isOwnMessage, nickname, message, ...rest }) {
    ...
    const messageIndex = usersMessages.findIndex(
      (x) => x.conversationId === rest[this.emitterOrReceiverAttr(isOwnMessage)]
    )
    const foundMessage = usersMessages.filter(
      (x) => x.conversationId === rest[this.emitterOrReceiverAttr(isOwnMessage)]
    )[0]
