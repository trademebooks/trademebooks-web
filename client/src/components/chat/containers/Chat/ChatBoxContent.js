...
class ChatBoxContent extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      params.id === prevProps.match.params.id &&
      this.checkIfUserHasMessages()
    ) {
      if (
        this.findUserMessagesObject(chatContext.state.usersMessages).messages
          .length !== prevState.userMessages.messages.length
      ) {
        this.setState(
          {
            userMessages: {
  ...
  getUserMessages() {
    ...
    const userMessages = this.findUserMessagesObject(
      chatContext.state.usersMessages
    )
  ...
  checkIfUserHasMessages() {
    ...
    return chatContext.state.usersMessages.some(
      (x) => x.conversationId === params.id
    )
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Chatbox innerRef={(element) => (this.messagesEnd = element)}>
